...
(define (unify-match p1 p2 frame)
  (cond ((eq? frame 'failed) 'failed)
        ((equal? p1 p2) frame)
        ((var? p1) (unify-if-possible p1 p2 frame))
        ((var? p2) (unify-if-possible p2 p1 frame));{\em ; ***}
        ((and (pair? p1) (pair? p2))
         (unify-match (cdr p1)
                      (cdr p2)
                      (unify-match (car p1)
...
(define (initialize-data-base rules-and-assertions)
  ...
  (put 'lisp-value 'qeval lisp-value)
  (put 'always-true 'qeval always-true)
  (put 'unique 'qeval uniquely-asserted) ; add unique
...
;; work4.63
;  根据先匹配再合一的实现,以下程序应该没有问题,等到实现query再行验证
(rule (grandson ?gf ?gs)
      (and (son ?gf ?f)
           (son ?f ?gs)))
